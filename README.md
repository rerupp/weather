...
* the [python](python/README.md) folder has Python code
* the [rust](rust/README.md) folder has the Rust code.

The `PyO3` bindings are in the [py_lib](rust/weather/py_lib/README.md) Rust module. For a period of 
time they were within the `python` folder but that didn't feel right. Refer to the `py_lib` module
for instructions on how to create the Python package containing the API bindings.

## Getting Started

Right now the easiest way to start is build the Rust [weather](rust/weather/README.md) project.
Once that has been built there is the `weather` CLI that can be used to bootstrap the environment.
You will then need to visit [visualcrossing](https://www.visualcrossing.com/) and get an API key.
There is a "sign up for free" link that will let you retrieve up to 1000 days of historical data
per calendar day.
