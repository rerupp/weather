...
pub fn execute(weather_data: &WeatherData, args: ArgMatches) -> cli::Result<()> {
    ...
    match weather_data.get_location(LocationFilter::alias(alias)) {
        Err(error) => cli::err!("Error getting location '{alias}' properties: {error:?}."),
        Ok(None) => cli::err!("Location '{alias}' was not found."),
        Ok(Some(location)) => {
            ...
            let future = weather_data.new_daily_histories(LocationFilter::alias(&location.alias), date_range)?;
            match get_histories(future)? {
                None => cli::err!("No daily histories found for {location}."),
