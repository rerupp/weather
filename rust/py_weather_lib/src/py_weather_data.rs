//! The `Python` class that front ends [weather data](WeatherData).
//!
//! The `__pyfunction_create` function is generated when creating the `PyO3` bindings.
...
/// The Python class that frontends access to the Rust [weather data](WeatherData) `API`.
///
#[pyclass]
pub struct PyWeatherData(
    /// The Rust [weather data](WeatherData) instance that will be called by the Python class.
    WeatherData
);
#[pymethods]
impl PyWeatherData {
    ...
    /// * `filters` identifies the locations.
    ///
    pub fn get_history_summaries(&self, filters: PyLocationFilters) -> PyResult<Vec<PyHistorySummary>> {
        elapsed_timer!("get_history_summary");
        match self.0.get_history_summaries(filters.into()) {
    ...
    /// * `limit` restricts the number of cities that will be returned..
    ///
    pub fn get_cities(&self, city_filter: PyLocationFilter, limit: usize) -> PyResult<Vec<PyLocation>> {
        elapsed_timer!("get_cities");
        let filter = LocationFilter {
            alias: None,
            city: city_filter.city,
