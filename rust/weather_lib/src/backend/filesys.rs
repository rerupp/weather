//! The [weather data](crate::weather_data) implementation that uses `ZIP` archives to
...
use crate::{
    backend::Backend,
    prelude::{Configuration, DailyHistories, DateRange, HistoryDates, HistorySummary, Location, LocationFilter},
};
use std::sync::Arc;
...
/// A bean providing metrics about a weather history file in the filesystem.
///
#[derive(Debug)]
pub struct FilesysMetadata {
    /// The location alias name.
    pub alias: String,
    /// The date associated with the history file.
    pub date: chrono::NaiveDate,
    /// The file disk size when not compressed.
    pub uncompressed_size: u64,
    /// The compressed file size in bytes.
    pub compressed_size: u64,
    /// The size of the file in the archive (metadata + content).
    pub data_size: u64,
}
...
impl Backend for ArchiveBackend {
    ...
    ///
    fn add_daily_histories(&self, mut daily_histories: DailyHistories) -> crate::Result<usize> {
        crate::log_elapsed_time!(trace, "add_daily_histories");
        let additions = fs_lib::daily_history::add(&self.weather_dir, &mut daily_histories)?;
        Ok(additions.len())
    }
    ...
    fn get_history_dates(&self, filters_opt: Option<Vec<LocationFilter>>) -> crate::Result<Vec<HistoryDates>> {
        ...
        let locations = fs_lib::get_locations(&self.weather_dir, filters_opt)?;
    ...
    /// # Arguments
    ///
    /// * `filters_opt` identifies the locations that should be used.
    ///
    fn get_history_summaries(&self, filters_opt: Option<Vec<LocationFilter>>) -> crate::Result<Vec<HistorySummary>> {
        crate::log_elapsed_time!(trace, "get_history_summary");
        let locations = fs_lib::get_locations(&self.weather_dir, filters_opt)?;
        fs_lib::history_metadata::get(&self.weather_dir, locations, self.configuration.weather_data.max_workers)
    }
    ...
    fn get_locations(&self, filters_opt: Option<Vec<LocationFilter>>) -> crate::Result<Vec<Location>> {
        ...
        fs_lib::get_locations(&self.weather_dir, filters_opt)
    }
