//! The implementation of weather data administration for filesystems.
//!

use crate::{
    admin_prelude::{FilesysDetails, FilesysProblems},
    backend::filesys::{fs_lib, history_archive::HistoryArchive, Locations, WeatherDir},
};
use std::fmt::Formatter;

/// Create an instance of the [FsAdmin] API.
///
/// # Arguments
///
/// * `weather_dir` is the weather data directory.
/// 
pub(in crate::backend) fn create_fs_admin(weather_dir: WeatherDir) -> FsAdmin {
    FsAdmin { weather_dir }
}
...
/// The filesystem administration API.
///
pub(crate) struct FsAdmin {
    /// The weather history directory.
...
impl FsAdmin {
    ...
    pub fn details(&self) -> crate::Result<FilesysDetails> {
        let locations = fs_lib::get_locations(&self.weather_dir, None)?;
        let archive_details = fs_lib::history_filesys_details::get(&self.weather_dir, locations, None)?;
        let mut archives_size = 0;
        let location_details = archive_details
            .into_iter()
            .map(|(location_details, archive_size)| {
                archives_size += archive_size;
                location_details
            })
            .collect();
        Ok(FilesysDetails { size: archives_size, location_details })
    }
...
mod internal {
    ...
    impl DataScanner {
        ...
        ///
        pub fn new(weather_dir: WeatherDir, repair: bool) -> Result<Self, FilesysDocumentProblem> {
