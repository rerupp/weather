//! This module frontends interacting with a ZIP archive.
//!
//! All zip file details are contained in this module. It is a complete rewrite of
//! the previous archive file implementation. When updating to the latest version
//! the zip crate it highlighted how spread out the previous implementation has
//! become.
//!
use super::FilesysMetadata;
...
impl ArchiveFile {
    ...
    ///
    pub fn metadata_iter(&self, selector: Option<&DateRange>) -> crate::Result<impl Iterator<Item = FilesysMetadata>> {
    ...
    pub fn metadata_by_date(
        ...
        skip_not_found: bool,
    ) -> crate::Result<impl Iterator<Item = FilesysMetadata>> {
        let archive = ArchiveReader::open(&self.lid, &self.file)?;
        if skip_not_found {
...
pub struct ArchiveContent {
    ...
    pub metadata: FilesysMetadata,
    /// The archive file data.
    pub data: ArchiveData,
}
/// Convert the archive content into metadata and History.
///
impl TryFrom<ArchiveContent> for (FilesysMetadata, History) {
    type Error = crate::Error;
    fn try_from(content: ArchiveContent) -> Result<Self, Self::Error> {
        let history: History = content.data.try_into()?;
        Ok((content.metadata, history))
    }
}
