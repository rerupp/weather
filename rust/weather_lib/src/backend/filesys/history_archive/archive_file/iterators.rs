...
use super::{archive, ArchiveContent, ArchiveData, FilesysMetadata};
...
impl<R: Read + Seek> ArchiveIterator<R> {
    ...
    /// log file.
    ///
    pub fn try_next(&mut self) -> Option<(NaiveDate, ZipFile<'_, R>)> {
        let mut item = None;
...
/// Creates the FilesysMetadata from the ZIP file
///
/// # Arguments
///
...
macro_rules! metadata {
    ($alias: expr, $date: ident, $zipfile: ident) => {{
        let compressed_size = $zipfile.compressed_size();
        FilesysMetadata {
            alias: $alias.to_string(),
            date: $date.clone(),
            uncompressed_size: $zipfile.size(),
            compressed_size,
            // each ZipFile has 30 bytes worth of metadata + the filename len
            data_size: compressed_size + 30 + $zipfile.name().len() as u64,
        }
    }};
}
...
impl<R: Read + Seek> ArchiveMetadataIterator<R> {
    ...
    ///
    pub fn new(lid: &str, archive: ZipArchive<R>, dates: Vec<NaiveDate>) -> Self {
        Self { lid: lid.to_string(), archive: ArchiveIterator::new(lid, archive, dates) }
    }
}
...
impl<R: Read + Seek> Iterator for ArchiveMetadataIterator<R> {
    type Item = FilesysMetadata;

    fn next(&mut self) -> Option<Self::Item> {
        let mut archive_metadata: Option<FilesysMetadata> = None;
        if let Some((date, zipfile)) = self.archive.try_next() {
            archive_metadata.replace(metadata!(self.lid, date, zipfile));
        }
        archive_metadata
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.get_next {
            if let Some((date, mut zipfile)) = self.archive.try_next() {
                match ArchiveData::new(&self.lid, &date, &mut zipfile) {
                    Ok(data) => {
                        let metadata = metadata!(self.lid, date, zipfile);
